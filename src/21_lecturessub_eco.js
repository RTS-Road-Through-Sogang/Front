...
const LecturesSubEco = () => {
  const [data_subecogicho, setData] = useState([]);
  const [data_subecoduty, setData2] = useState([]);
  const [data_subecodutychoice, setData3] = useState([]);
  const [data_subecochoice, setData4] = useState([]);
  ...
  function processSubEco(dataObject) {
    ...
    return processedSubEco;
  }

  // 데이터 처리 함수를 이용하여 데이터 합치기
  const combinedDataSub = [
    ...processSubEco(data_subecogicho),
    ...processSubEco(data_subecoduty),
    ...processSubEco(data_subecodutychoice),
    ...processSubEco(data_subecochoice),
  ];
