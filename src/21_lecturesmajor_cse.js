...
const LecturesMajorCse = () => {
  const [data_majorcsegicho, setData] = useState([]);
  const [data_majorcseduty, setData2] = useState([]);
  const [data_majorcsedutychoice, setData3] = useState([]);
  const [data_majorcsechoice, setData4] = useState([]);
  ...
  function processMajorCse(dataObject) {
    ...
    return processedMajorCse;
  }

  // 데이터 처리 함수를 이용하여 데이터 합치기
  const combinedDataMajor = [
    ...processMajorCse(data_majorcsegicho),
    ...processMajorCse(data_majorcseduty),
    ...processMajorCse(data_majorcsedutychoice),
    ...processMajorCse(data_majorcsechoice),
  ];
...
export default LecturesMajorCse;
